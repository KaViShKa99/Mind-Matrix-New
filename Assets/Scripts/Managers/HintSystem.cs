...
public class HintSystem : MonoBehaviour
{
    ...
    // Note: Change this to AStarSolver if you prefer to rename the file.
    private AStarSolver solver; 

    // Make sure your MinHeap is available to the AStarSolver!

    public void Initialize(TileManager manager, int gridSize)
    {
        tileManager = manager;
        this.gridSize = gridSize;
        // Instantiate the AStarSolver
        solver = new AStarSolver(gridSize); 
    }

    /////////////////////////////////////////////////////////
    ///////////////// UNDO METHODS (Unchanged) ///////////////
    /////////////////////////////////////////////////////////
    ...
    private IEnumerator AutoPlaceTiles(int stepsToPlay)
    {
        ...
        float solverWeight = 1f; // Default to optimal A*

        if (gridSize >= 5)
        {
            // For 5x5 and larger, use a weighted heuristic for speed (sub-optimal path)
            solverWeight = 5f; 
        }

        // Use the optimized AStarSolver
        List<int[]> path = solver.Solve(start, goal, solverWeight);
        ...
        for (int i = 1; i < path.Count && placed < stepsToPlay; i++)
        {
            ...
            // Find which tile moved *into* the empty spot
            int emptyPrev = Array.IndexOf(prev, 0);
            int emptyNext = Array.IndexOf(next, 0);
            int movedTileIndex = emptyNext; // The position of the tile that moved

            Transform tile = tileManager.GetTileAtIndex(movedTileIndex);
            if (tile == null) continue;

            // This triggers the TileManager's move logic and updates the board state
            tileManager.TryMoveTile(tile); 
            placed++;
    ...
    private IEnumerator HintCoroutine()
    {
        ...
        // Use the optimized AStarSolver with weight
        List<int[]> path = solver.Solve(start, goal, solverWeight);
        ...
        for (int i = 1; i < path.Count && shown < maxHintSteps; i++, shown++)
        {
            ...
            // ... (rest of the hint code is the same) ...
